...
il faut dans un 1er temps actibver le module de sécurité *secirutyk9*. Sur les 2 routeurs effectuer ces commandes :
...
Après avoir configuré les 2 routeurs, les 2 machines aux extrémités du réseau devraient pouvoir se pinger entre elles et communiquer de manière cryptée. Si tel n'est pas le cas, on peut effectuer un *sh run* afin de vérifier s'il n'y a pas un message du style : *configuration incomplete*. 

### sh crypto map

On peut débugger avec la commande *sh crypto map* qui doit nous renvoyer ces résultats de la configuration courante :
...
Les commandes *show crypto isakmp policy* et *show crypto isakmp sa* permettent d'afficher les informations par rapport au protocole ISAKMP avec SA pour *Security Association*.
