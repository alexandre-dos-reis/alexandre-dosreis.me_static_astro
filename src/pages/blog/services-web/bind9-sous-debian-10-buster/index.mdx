...
Nous allons configurer un serveur DNS secondaire qui prendra la relève si jamais notre serveur DNS primaire est inaccessible. Il faut reprendre ce tutoriel depuis la [Prérequis](#prérequis) en changeant le nom d'hôte par *dns02* et avec l'adresse IP *172.16.200.2*. Le principal changement viendra du fichier de zone */etc/bind/named.conf.local* que l'on modifiera comme suit :
